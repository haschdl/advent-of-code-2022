...
let input = "input/day14-input.txt"
...
let startT = Date.now();
var outFile = fs.createWriteStream('day14-intermediates.b.txt')
...
for (let i = 0; i < 160 * 160 * 10; i++) {
    ...
    const floorLine = line(new p(minX, floorY), new p(maxX, floorY));
    floorLine.forEach(p => p.setC("="))

    const shouldExport = i<1000 ? i %50==0 : (i < 5000 ? i %250==0 : i % 1000==0);
    if (shouldExport) {
        let b = drawArrayOffSet([...grid, ...floorLine]);

        b.split('\n').forEach(line => {
            let bytes: number[] = [];
            line.split("").forEach(c => bytes.push([".", "#", "o", "="].indexOf(c)))
            bytes.push(4);//4 for \n
            outFile.write(new Uint8Array(bytes));
        });
        outFile.write(new Uint8Array([5])); //"frame separator"
    }
...
console.log("Resting sand count (part 2)", t - r0); //27566
assert(27566 == t - r0)