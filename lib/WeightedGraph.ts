...
class PriorityQueue {
    ...
    sinkDown() {
        ...
        while (true) {
            ...
            let leftChild: Node|undefined=undefined;
            let rightChild: Node;
            let swap = -1;
...
export class WeightedGraph {
    ...
    Dijkstra(start: number, finish: number) {
        const nodes = new PriorityQueue();
        const distances = {};
        const previous = {};
        let path: number[] = []; //to return at end
        let smallest: number = Infinity;
        //build up initial state
        for (let vertexKey in this.adjacencyList) {
            let vertex = Number(vertexKey);
            if (vertex === start) {
                distances[vertex] = 0;
